...
class Upload extends Component {
  ...
  upload = event => {
    ...
    if (this.state.image === "") {
      this.setState({
        upload_error: true,
        upload_error_msg: "No image is being uploaded!"
      });
      return;
    }
    ...
    fetch(API_URL + "/api/upload", {
      ...
      .then(responseJson => {
        if (responseJson.success) {
          ...
          this.setState({
            upload_error: true,
            upload_error_msg: responseJson.message
          });
        }
      });
  };
  ...
  render() {
    const errmsg = this.state.upload_error ? (
      <Alert bsStyle="danger">{this.state.upload_error_msg}</Alert>
    ) : (
      <div />
    );
